...
void frd(double *co,ITG *nk,ITG *kon,ITG *ipkon,char *lakon,ITG *ne0,
  ...
  ITG ncomptensor=6,ifieldtensor[6]={1,1,1,1,1,1},icomptensor[6]={0,1,2,3,5,4},
      nfieldtensor[2]={6,0};
  ITG ncomprdp=8,ifieldrdp[8]={1,1,1,1,1,1,1,1},icomprdp[8]={0,1,2,3,4,6,5,7},
      nfieldrdp[2]={8,0};
  ITG ncompdrx=5,ifielddrx[5]={1,1,1,1,1},icompdrx[5]={0,1,2,3,4},
      nfielddrx[2]={5,0};
  ITG ncompdmg=2,ifielddmg[2]={1,1},icompdmg[2]={0,1},
      nfielddmg[2]={2,0};
  ITG ncompinv=6,ifieldinv[6]={1,1,1,1,1,1},icompinv[6]={0,1,2,3,4,5},
      nfieldinv[2]={6,0};
  ...
  double pi,oner,rdp[8**nk],drx[5**nk],dmg[2**nk];
...
//	  /* MASS: store nothing */
  ...
  /* storing the rate dependent plastics in the nodes */

  if((*nmethod!=5)||(*mode==-1)){
      if((strcmp1(&filab[4089],"RDP ")==0)&&(*ithermal!=2)){
	  iselect=1;

	  frdset(&filab[4089],set,&iset,istartset,iendset,ialset,
		 inum,&noutloc,&nout,nset,&noutmin,&noutplus,&iselect,
		 ngraph);
	  ...
	  fprintf(f1," -4  PLAS        8    1\n");
	  fprintf(f1," -5  PEEQ        1    4    0    0\n");
	  fprintf(f1," -5  PEXX        1    4    1    1\n");
	  fprintf(f1," -5  PEYY        1    4    2    2\n");
	  fprintf(f1," -5  PEZZ        1    4    3    3\n");
	  fprintf(f1," -5  PEXY        1    4    1    2\n");
	  fprintf(f1," -5  PEYZ        1    4    2    3\n");
	  fprintf(f1," -5  PEZX        1    4    3    1\n");
	  fprintf(f1," -5  PERT        1    4    0    0\n");

          for(i=0;i<*nk;i++){
	     for(j=0;j<8;j++){
                rdp[j+i*8]=xstaten[j+i**nstate_];
             }
	  }

	  frdselect(rdp,rdp,&iset,&nkcoords,inum,m1,istartset,iendset,
		    ialset,ngraph,&ncomprdp,ifieldrdp,icomprdp,
		    nfieldrdp,&iselect,m2,f1,output,m3);
      }

  }
  
  /* storing the dynamic recrystallization in the nodes */

  if((*nmethod!=5)||(*mode==-1)){
      if((strcmp1(&filab[4176],"DRX ")==0)&&(*ithermal!=2)){
	  iselect=1;

	  frdset(&filab[4176],set,&iset,istartset,iendset,ialset,
		 inum,&noutloc,&nout,nset,&noutmin,&noutplus,&iselect,
		 ngraph);
	  ...
	  fprintf(f1," -4  DRX         5    1\n");
	  fprintf(f1," -5  FRACTION    1    4    0    0\n");
	  fprintf(f1," -5  DRX_SIZE    1    4    0    0\n");
	  fprintf(f1," -5  AVE_SIZE    1    4    0    0\n");
	  fprintf(f1," -5  AVE_TEMP    1    4    0    0\n");
	  fprintf(f1," -5  AVE_RATE    1    4    0    0\n");

          for(i=0;i<*nk;i++){
	     for(j=0;j<5;j++){
                drx[j+i*5]=xstaten[10+j+i**nstate_];
             }
	  }

	  frdselect(drx,drx,&iset,&nkcoords,inum,m1,istartset,iendset,
		    ialset,ngraph,&ncompdrx,ifielddrx,icompdrx,
		    nfielddrx,&iselect,m2,f1,output,m3);
      }

  }
  
  /* storing the damage in the nodes */

  if((*nmethod!=5)||(*mode==-1)){
      if((strcmp1(&filab[4263],"DMG ")==0)&&(*ithermal!=2)){
	  iselect=1;

	  frdset(&filab[4263],set,&iset,istartset,iendset,ialset,
		 inum,&noutloc,&nout,nset,&noutmin,&noutplus,&iselect,
		 ngraph);
	  ...
	  fprintf(f1," -4  DAMAGE      2    1\n");
	  fprintf(f1," -5  DAMAGE      1    4    0    0\n");
	  fprintf(f1," -5  D_STRAIN    1    4    0    0\n");

          for(i=0;i<*nk;i++){
	     for(j=0;j<2;j++){
                dmg[j+i*2]=xstaten[15+j+i**nstate_];
             }
	  }

	  frdselect(dmg,dmg,&iset,&nkcoords,inum,m1,istartset,iendset,
		    ialset,ngraph,&ncompdmg,ifielddmg,icompdmg,
		    nfielddmg,&iselect,m2,f1,output,m3);
      }
...
/*
// stoVring the stress invariants in the nodes
	  ...
	  fprintf(f1," -4  STRESS_INV  6    1\n");
	  fprintf(f1," -5  PRESSURE    1    1    0    0\n");
	  fprintf(f1," -5  MISES       1    1    0    0\n");
	  fprintf(f1," -5  TRESCA      1    1    0    0\n");
	  fprintf(f1," -5  1ST STRESS  1    1    0    0\n");
	  fprintf(f1," -5  2ND STRESS  1    1    0    0\n");
	  fprintf(f1," -5  3RD STRESS  1    1    0    0\n");

	  FORTRAN(invariant,(nk,stn,sinv,&ncompinv,icompinv));

	  frdselect(sinv,sinv,&iset,&nkcoords,inum,m1,istartset,iendset,
		    ialset,ngraph,&ncompinv,ifieldinv,icompinv,
		    nfieldinv,&iselect,m2,f1,output,m3);
*/
      }
