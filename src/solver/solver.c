...
/* _set_output_format is MSVC-specific, not needed on MinGW */
...
ITG *kon=NULL, *nodeboun=NULL, *ndirboun=NULL, *ipompc=NULL,
    ...
    *ncocon=NULL,*ibody=NULL,*ielprop=NULL,*islavsurf=NULL,*nphase=NULL,
...
ITG nk,ne,nboun,nmpc,nforc,nload,nprint=0,nset,nalset,nentries=22,
  ...
  *ikmpcref=NULL,isens=0,namtot=0,nstam=0,ndamp=0;
...
double *co=NULL, *xboun=NULL, *coefmpc=NULL, *xforc=NULL,*clearini=NULL,
	...
	*xstate=NULL, *trab=NULL, *ener=NULL, *shcon=NULL, *cocon=NULL,
        ...
        *xbody=NULL,*xbodyold=NULL,*coefmpcref=NULL,*dacon=NULL,*mpcon=NULL,
        *pphase=NULL,*cphase=NULL,*phaseother=NULL,*flcon=NULL,*lh=NULL,enthactrl[6]={-1.,-1.,-1.,-1.,-1.,1.},
        *rdpcon=NULL,*gscon=NULL;
    
double ctrl[53]={4.5,8.5,9.5,16.5,10.5,4.5,0.,5.5,0.,0.,0.25,0.5,0.75,0.85,0.,0.,1.5,0.,0.005,0.01,0.,0.,0.02,1.e-5,1.e-3,1.e-8,1.e30,1.5,0.25,1.01,1.,1.,5.e-7,5.e-7,5.e-7,5.e-7,5.e-7,5.e-7,5.e-7,-1.,1.e20,1.e20,1.e20,1.e20,1.e20,1.e20,1.e20,1.5,0.5,20.5,0.5,1.5,0.};
...
FORTRAN(allocation,(&nload_,&nforc_,&nboun_,&nk_,&ne_,&nmpc_,&nset_,&nalset_,
   ...
   iuel,&iprestr,&nstam,&ndamp,&nmpmat_,phase_inf));
...
//    nobject=nobject_;
    ...
    /* metal powder constants */

    NNEW(mpcon,double,2*nmpmat_*nmat);
    NNEW(nmpcon,ITG,2*nmat);

    /* rate-dependent plastic */
    NNEW(rdpcon,double,10*3*nmat);

    /* rate-dependent plastic */
    NNEW(gscon,double,30*5*nmat);
    ...
    NNEW(flcon,double,2*ntmat_*nmat);
    NNEW(nflcon,ITG,nmat);

    /* latent heat */
    NNEW(lh,double,nmat);

    /* material id on both sides of a contact pair */
    NNEW(ipairmat,ITG,3*nmat);
    ...
    if(istep==0){
      ...
      DMEMSET(t0,0,nk_, 0.0);
      DMEMSET(t1,0,nk_, 0.0);
    }
  ...
  FORTRAN(calinput,(co,&nk,kon,ipkon,lakon,&nkon,&ne,
	    ...
	    &maxlenmpcref,&memmpc_,&isens,&namtot,&nstam,dacon,
	    mpcon,nmpcon,&nmpmat_,pphase,cphase,phaseother,nphase,phase_inf,
      nflcon,flcon,lh,enthactrl,ipairmat,rdpcon,gscon));

#ifdef WeICME_EXTERNAL_BEHAVIOURS_SUPPORT
  for(i=0;i!=nmat;++i){
    WeICME_registerExternalBehaviour(matname+80*i);
  }
...
//	FORTRAN(writeboun,(nodeboun,ndirboun,xboun,typeboun,&nboun));
  ...
  /* tied contact constraints: generate appropriate MPC's */
    ...
    RENEW(mpcon,double,2*nmpmat_*nmat);
    RENEW(nmpcon,ITG,2*nmat);

    RENEW(rdpcon,double,10*3*nmat);
    
    RENEW(gscon,double,30*5*nmat);
    ...
    RENEW(flcon,double,2*ntmat_*nmat);
    RENEW(nflcon,ITG,nmat);

    RENEW(lh,double,nmat);

    RENEW(ipairmat,ITG,3*nmat);
  ...
  FORTRAN(phaseini,(xstate,&nstate_,mi,&ne,lakon,cphase,phase_inf,nphase));
...
//  if(icascade==0) {
  ...
  if((nmethod<=1)||(nmethod==11)||((iperturb[0]>1)&&(nmethod<8)))
	...
	linstatic(co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,ndirboun,xboun,&nboun, 
	     ...
	     orname,mpcon,nmpcon,&nmpmat_,pphase,cphase,phaseother,nphase,phase_inf);
	...
	memmpc_=mpcinfo[0];mpcfree=mpcinfo[1];icascade=mpcinfo[2];
      ...
      else if(enthactrl[1]<0 && ctrl[52]<1e-3){
	...
	nonlingeo(&co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,ndirboun,xboun,&nboun, 
	     ...
	     xmodal,&iaxial,&inext,&nprop,&network,orname,mpcon,nmpcon,
	     &nmpmat_,pphase,cphase,phaseother,nphase,phase_inf,flcon,nflcon,lh,
	     rdpcon,gscon);
	...
	for(i=0;i<3;i++){nzsprevstep[i]=nzs[i];}
      ...
      } else if(ctrl[52]<1e-3){

        mpcinfo[0]=memmpc_;mpcinfo[1]=mpcfree;mpcinfo[2]=icascade;
        mpcinfo[3]=maxlenmpc;

        solidification(&co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,ndirboun,xboun,&nboun, 
            &ipompc,&nodempc,&coefmpc,&labmpc,&nmpc,nodeforc,ndirforc,xforc,
                  &nforc,&nelemload,&sideload,xload,&nload, 
            nactdof,&icol,jq,&irow,neq,&nzl,&nmethod,&ikmpc, 
            &ilmpc,ikboun,ilboun,elcon,nelcon,rhcon,nrhcon,
            ...
            &ifacecount,typeboun,&islavsurf,&pslavsurf,&clearini,&nmat,
            xmodal,&iaxial,&inext,&nprop,&network,orname,mpcon,nmpcon,
            &nmpmat_,pphase,cphase,phaseother,nphase,phase_inf,flcon,nflcon,
            lh,enthactrl,ipairmat);
	...
	castfvm(&co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,ndirboun,xboun,&nboun, 
	     &ipompc,&nodempc,&coefmpc,&labmpc,&nmpc,nodeforc,ndirforc,xforc,
             &nforc,&nelemload,&sideload,xload,&nload, 
	     nactdof,&icol,jq,&irow,neq,&nzl,&nmethod,&ikmpc, 
	     &ilmpc,ikboun,ilboun,elcon,nelcon,rhcon,nrhcon,
  ...
  else if(nmethod==5)
    {
      ...
      steadystate(&co,&nk,&kon,&ipkon,&lakon,&ne,&nodeboun,&ndirboun,&xboun,&nboun,
	    ...
	    &ndamp,dacon,mpcon,nmpcon,&nmpmat_,pphase,cphase,phaseother,nphase,phase_inf,
	    rdpcon,gscon);
    }
	...
	electromagnetics(&co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,
	     ...
	     &network,orname,mpcon,nmpcon,&nmpmat_,pphase,cphase,phaseother,nphase,phase_inf);
	...
	sensitivity(co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,ndirboun,
	     ...
	     &nobject,&objectset,&istat,orname,nzsprevstep,&nlabel,physcon,
             jobnamef,mpcon,nmpcon,&nmpmat_,rdpcon,gscon);
  }
...
SFREE(flcon);SFREE(nflcon);SFREE(lh);SFREE(ipairmat);
